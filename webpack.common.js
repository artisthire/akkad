...
module.exports = (_, argv) => ({
  ...
  module: {
    rules: [
      ...
      {
        // PUG
        // используется simple-pug-loader как более новая замена pug-loader
        test: /\.pug$/i,
        use: [
          {
  ...
  plugins: [
    ...
    new MiniCssExtractPlugin({
      ...
      // prettier-ignore
      filename: (pathData) =>
        (pathData.chunk.name.includes('.min') ? `${PATHS.assets}css/style.min.css` : `${PATHS.assets}css/style.css`),
    }),
    ...
    new CopyPlugin({
      patterns: [
        ...
        {from: `${PATHS.src}/assets/img`, to: `${PATHS.assets}/img/[path][name][ext]`},
